...
| `Competition` | **Aggregate Root** | Inscription à une compétition — mutable, distinct de `Bracket` (voir ADR 007). `create()` (avec `BracketConfiguration`, choisie une fois pour toutes à la création, et `OrganizationId`, voir ADR 029), `register()`/`withdraw()` (uniquement tant que l'inscription est ouverte), `closeRegistration()` (échoue sous `minTeams`), `generateBracket(BracketGeneratorFactory)` (résout son propre générateur via `$factory->forConfiguration($this->bracketConfiguration)` — protège l'invariant format/générateur, action manuelle et distincte de la clôture, échoue si l'inscription est encore ouverte ou déjà générée) — voir ADR 026. `requestToJoinTeam()`/`approveJoinRequest()`/`rejectJoinRequest()` (même garde "inscription ouverte" que `register()`/`withdraw()`, déléguent à `Team`) + `getTeamPendingRequests()`/`getTeamRoster()` en lecture, pour le flux d'adhésion à une équipe. `$teams` est un `array<string, Team>` keyé par `TeamId::value` (accès direct, pas de scan linéaire). Règles de cohérence inter-équipes (voir ADR 032) : `register()` rejette un nom d'équipe déjà pris (comparaison normalisée, casse/espaces ignorés via `normalizeTeamName()`) et un capitaine déjà membre d'une autre équipe ; `requestToJoinTeam()`/`approveJoinRequest()` rejettent de même un joueur déjà membre (roster confirmé uniquement, pas les demandes en attente) d'une autre équipe — `approveJoinRequest()` revérifie à l'approbation (pas seulement à la demande) pour fermer la fenêtre de course entre deux candidatures concurrentes ; méthode privée commune `belongsToAnotherTeam(PlayerId, ?TeamId $excludingTeamId)` |
...
- `Http/LogicExceptionListener` — même patron, mappe toute `\LogicException` du domaine en `JsonResponse(['error' => ...], 409)`. Comble un gap laissé ouvert par ADR 015 (qui ne mappait que `\InvalidArgumentException`, `\LogicException` jugée alors non atteignable en HTTP) — devenu concret avec les règles de cohérence inter-équipes (ADR 032), et corrige au passage les règles préexistantes du même type (compétition pleine, équipe déjà inscrite) qui remontaient jusqu'ici en 500 brut.
...
- ADR 032 — Règles de cohérence inter-équipes : unicité du nom d'équipe par comparaison normalisée (casse/espaces ignorés, pas de comparaison stricte) ; "un joueur = une seule équipe" scopé au roster confirmé uniquement (pas les demandes en attente — candidature multiple autorisée), revérifié à l'approbation pour fermer la fenêtre de course entre deux candidatures concurrentes approuvées séparément ; libération immédiate au `withdraw()` sans nouvel état (comportement déjà existant de l'agrégat) ; a rendu concret un gap préexistant sur le mapping HTTP de `\LogicException` (jamais interceptée depuis ADR 015), corrigé par `LogicExceptionListener`/409 dans la foulée
...
**Règles de cohérence inter-équipes (Priorité 6) ✅ entièrement terminées** : unicité du nom d'équipe (comparaison normalisée) et un joueur ne peut appartenir qu'à une seule `Team` par `Competition` (roster confirmé, revérifié à l'approbation) — voir ADR 032. A aussi corrigé au passage un gap de mapping HTTP préexistant (`\LogicException` jamais interceptée, `LogicExceptionListener`/409 ajouté).

**Prochaine étape concrète en reprenant (reste de Priorité 6)** : flux "quitter une équipe" (départ volontaire/exclusion d'*un seul joueur* du roster, distinct de `Withdraw` qui retire toute l'équipe — cas du capitaine qui part, désignation d'un successeur, pas encore conçu). Voir détail dans `ROADMAP.md` (Priorité 6).
