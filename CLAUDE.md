...
Priorité 5b (persistance réelle) ✅ implémentée (voir `ROADMAP.md` pour le détail). Priorité 5c (API REST pour les autres use cases) bien avancée : `RegisterTeam` (inscription d'équipe, capitaine-seul), `CreatePlayer` (création d'identité joueur, `POST /players` — réponse anti-énumération, voir ADR 024), `Withdraw` (désistement, `DELETE /competitions/{id}/teams/{teamId}`) et `CloseRegistration` (clôture, `POST /competitions/{id}/close-registration`) ✅ faits bout en bout (Application + HTTP). Reste : `generateBracket` (génération du bracket), même patron CQRS que les use cases précédents. Le détail complet du plan est dans `ROADMAP.md`.