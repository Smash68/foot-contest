...
**En cours (Priorité 6, flux "rejoindre une équipe")** : couche Domain (`Team`/`Competition`) et use case `RequestToJoinTeam` (Application + HTTP) ✅ faits. **Prochaine étape concrète en reprenant** : `ApproveJoinRequest` (Command/Handler avec autorisation capitaine-seul — acteur unique, décision actée en session : pas de double acteur comme `Withdraw`, "c'est uniquement le capitaine qui gère son équipe" — nouvelle exception 403 mirroir `NotAuthorizedToWithdrawException`), un test à la fois en TDD strict, puis HTTP ; ensuite `RejectJoinRequest`, même patron. Voir détail dans `ROADMAP.md` (Priorité 6, sous-liste "Flux 'rejoindre une équipe'").
