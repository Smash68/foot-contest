...
**En cours (Priorité 6, flux "rejoindre une équipe")** : couche Domain (`Team`/`Competition`), `RequestToJoinTeam` et `ApproveJoinRequest` (Application + HTTP) ✅ faits — autorisation capitaine-seul (acteur unique, pas de double acteur comme `Withdraw`), rejet via `NotAuthorizedToManageJoinRequestException`/`NotAuthorizedExceptionListener` unifié (ADR 031). **Prochaine étape concrète en reprenant** : `RejectJoinRequest`, même patron qu'`ApproveJoinRequest` (autorisation capitaine-seul, réutilise `NotAuthorizedToManageJoinRequestException` — pas une nouvelle exception, la règle d'autorisation est identique), un test à la fois en TDD strict, Application puis HTTP (`POST /competitions/{id}/teams/{teamId}/join-requests/{playerId}/reject`, mirroir de la route `approve`). Voir détail dans `ROADMAP.md` (Priorité 6, sous-liste "Flux 'rejoindre une équipe'").

Reste après ça sur Priorité 6 : règles de cohérence inter-équipes (unicité du nom, un joueur par équipe) et flux "quitter une équipe" (départ volontaire/exclusion d'un roster, distinct de `Withdraw` — cas du capitaine à trancher) — voir `ROADMAP.md`. Choix du front (Vue 3 ou Twig) déplacé en toute dernière étape (Priorité 8), une fois l'API stabilisée. Le détail complet du plan est dans `ROADMAP.md`.