...
- Trois nouveaux cas d'erreur en `\LogicException` sur `register()`, `requestToJoinTeam()` et `approveJoinRequest()` : nom déjà pris, capitaine/joueur déjà dans une autre équipe. Ces règles rendaient pour la première fois concret un gap préexistant (ADR 015 ne mappait que `\InvalidArgumentException` en 422, `\LogicException` jamais interceptée en HTTP faute d'être atteignable à l'époque) — corrigé dans la foulée par `LogicExceptionListener` (mirroir exact d'`InvalidArgumentExceptionListener`), qui mappe désormais toute `\LogicException` du domaine en 409 Conflict. Bénéfice rétroactif : les règles préexistantes déjà en `\LogicException` (compétition pleine, équipe déjà inscrite, inscription close) sont elles aussi correctement mappées maintenant, sans changement de code sur ces méthodes.
